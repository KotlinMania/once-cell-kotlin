...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`once_cell`](https://crates.io/crates/once_cell).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `once_cell` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

One-time cell / lazy initializer (mostly merged into stdlib)
...
Original work copyrighted by the upstream `once_cell` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
