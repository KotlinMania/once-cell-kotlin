# Claude Code Project Instructions — once-cell-kotlin

## Project Overview

This is **once-cell-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`once_cell`](https://crates.io/crates/once_cell).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/once_cell/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/once_cell>
package io.github.kotlinmania.oncecell
```
